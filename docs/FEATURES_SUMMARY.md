...
| **Grounded chat** | **POST /chat** (and **`POST /api/embed/chat`**) run a registered **agent** with tenant-safe retrieval. **Source filenames are not shown in the assistant reply** to end users; they are recorded in the **Chat audit** log (`documents:read`). Optional **collection_ids** narrows which collections are searched. |
...
| **Console UI** | Landing, **`/docs`** (API reference), login, **dashboard**: Overview, Agents, Chat, **Chat audit** (transcripts + KB sources), Knowledge base, **Embed**, **Visitor leads** (embed contact submissions when `embed:keys`), **Team** when `users:manage`. Platform **`is_superuser`**: **Organisations**, **Platform settings**, **Product & deployment**, **Technical reference** (`/super/*`). |
| **Embeddable chat** | **Embed API keys** + **`/api/embed/chat`** + **`GET /api/embed/widget-config`** + optional **`POST /api/embed/visitor-contact`** + **`static/embed/nexura-chat.js`**. Org admins configure widget title, welcome text, contact gate, and engagement counter via **`/api/v1/tenant/embed-branding`**. CORS via **`EMBED_CORS_ORIGINS`** / per-key origins. |
| **Chat audit** | Every console and embed query outcome is logged (**`chat_query_audits`**): query, answer or error, KB **sources** (audit-only), channel (console vs embed key). Filter by **Console** or **embed key**; **`documents:write`** can permanently delete rows. |
| **Visitor leads** | When the embed contact form is enabled, submissions are stored (**`embed_visitor_leads`**) and listed/exported on the **Visitor leads** dashboard page (**`embed:keys`**). |
...
| **Personalisation** | **Allowlist** = which agents the org **may** run. **Tone / instructions** = per-user preference **`config`** (optional **`agent_display_names`** for console labels) and/or per-embed-key **`agent_config_json`**. **Widget branding / visitor gate**: **`Tenant`** **`embed_*`** columns · **`/api/v1/tenant/embed-branding`**. |
